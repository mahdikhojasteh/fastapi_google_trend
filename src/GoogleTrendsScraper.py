...
class GoogleTrendsScraper:
    ...
    def get_trends(self, keywords, start, end, region=None, category=None):
        ...
        return data, related_queries_list, related_topics_list
    ...
    def get_data(self, url):
        ...
        while button is None:
            try:
                ...
                timeout = 10
                line_chart = WebDriverWait(self.browser, timeout).until(
                    EC.presence_of_element_located((By.CSS_SELECTOR, "widget[type='fe_line_chart']"))
                )

                button = WebDriverWait(line_chart, timeout).until(
    ...
    def get_related_topics(self, url):
        ...
        while button is None:
            try:
                ...
                time.sleep(self.sleep + 2 * np.random.rand())
                # Try to find the button and click it
                xpath = '//*[@widget-name="RELATED_TOPICS"]//*[@class="widget-actions-item export"]'
                timeout = 10
    ...
    def get_related_queries(self, url):
        ...
        while button is None:
            try:
                ...
                time.sleep(self.sleep + 2 * np.random.rand())
                # Try to find the button and click it
                xpath = '//*[@widget-name="RELATED_QUERIES"]//*[@class="widget-actions-item export"]'
                timeout = 10
                button = WebDriverWait(self.browser, timeout).until(
                    EC.presence_of_element_located((By.XPATH, xpath))
                )
